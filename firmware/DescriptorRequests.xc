...
int DescriptorRequests(XUD_ep c, XUD_ep c_in, uint8 devDesc[], int devDescLength, uint8 cfgDesc[], int cfgDescLength,
    uint8 devQualDesc[], int devQualDescLength, uint8 oSpeedCfgDesc[], int oSpeedCfgDescLength, uint8 strDescs[][40], SetupPacket &sp)
{
     /* Return value */
    int datalength;
    int stringID = 0;
    ...
    /* Wait for Setup data */
    if (XUD_GetSetupBuffer(c, buffer) < 0) 
    {
        return -1;
    } else {

    /* Parse data buffer end populate SetupPacket struct */
    XUD_ParseSetupPacket(buffer, sp);
